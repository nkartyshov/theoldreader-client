...
interface ArticleDao {
    ...
    @Query("select a.*, s.title as subscription_title from articles a inner join subscriptions s on a.subscription_id = s.id where deleted = 0 order by a.publish_date desc")
    fun findAll(): List<ArticleListItem>

    @Query("select a.*, s.title as subscription_title from articles a inner join subscriptions s on a.subscription_id = s.id where a.subscription_id = :subscriptionId and deleted = 0 order by a.publish_date desc")
    fun findAll(subscriptionId: String?): List<ArticleListItem>

    @Query("select a.*, s.title as subscription_title from articles a inner join subscriptions s on a.subscription_id = s.id where a.read = 0 and deleted = 0 order by a.publish_date desc")
    fun findUnread(): List<ArticleListItem>

    @Query("select a.*, s.title as subscription_title from articles a inner join subscriptions s on a.subscription_id = s.id where a.subscription_id = :subscriptionId and a.read = 0 and deleted = 0 order by a.publish_date desc")
    fun findUnread(subscriptionId: String?): List<ArticleListItem>

    @Query("select a.*, s.title as subscription_title from articles a left join subscriptions s on a.subscription_id = s.id where a.favorite = 1 and deleted = 0 order by a.publish_date desc")
